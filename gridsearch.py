...
def main():
    ...
    # '''move------------------------------''' 
    # if alg_name is 'FW':
    #     training_features = training_features_l1
    #     testing_features = testing_features_l1
    ...
    if len(sys.argv) > 4:
        ...
        # timefile = open(os.path.join(result_location, alg_name+'_'+dataset_name+'_'+sys.argv[3]+'.time'), 'w')
        ...
        # timefile = open(os.path.join(result_location, alg_name+'_'+dataset_name+'.time'), 'w')
    ...
    for alg_name in alg_name_list:
        ...
        for i, eps in enumerate(eps_list):
            for j, hyper in enumerate(hypers):
                ...
                # for k in hyper.values():
                #     print('\t{0}'.format(k), end='')
                #     print('\t{0}'.format(k), end='', file=logfile)
